...
try:
    ...
    from tools.nuscenes.utils.geometry_utils import transform_matrix
    from tools.nuscenes.eval.detection.config import config_factory
    from tools.nuscenes.eval.detection.evaluate_nia import NuScenesEval
except:
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=10, filter_zero=True):
    ...
    for sample in tqdm(nusc.sample):
        ...
        all_cams_from_lidar, all_cams_intrinsic, all_cams_path = get_lidar_to_image_transform(nusc,
                                                                                              pointsensor=ref_sd_rec,
                                                                                              camera_sensor=ref_cams)
        ...
        while len(sweeps) < nsweeps - 1:
            if curr_sd_rec["prev"] == "":
                ...
                cur_cams_from_lidar, cur_cams_intrinsic, cur_cams_path = get_lidar_to_image_transform(nusc,
                                                                                                      pointsensor=curr_sd_rec,
                                                                                                      camera_sensor=cam_data)
