...
def read_file(path, tries=2, num_point_feature=4, virtual=False, calib=''):
    if virtual:
        ...
        if 'Lidar' in path:
            ...
            if 'S_Clip' in path:
                R2L_transform = np.array([[-0.015895, 0.999858, -0.005580, 2.592000],
                                          [-0.999874, -0.015891, 0.000646, 0.003550],
                                          [0.000557, 0.005590, 0.999984, -1.805720]])
            elif 'A_Clip' in path:
                R2L_transform = np.array([[-0.007810, 0.999931, -0.008789, 2.208720],
                                          [-0.999965, -0.007783, 0.003169, 0.208666],
                                          [0.003101, 0.008813, 0.999956, -1.731045]])
            pts = open3d.t.io.read_point_cloud(path)
            positions = pts.point['positions'].numpy().transpose(1, 0)
            intensity = pts.point['intensity'].numpy()
            curvature = pts.point['curvature'].numpy()
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            if nsweeps > 1:
                ...
                res["lidar"]["times"] = times
                res["lidar"]["combined"] = np.hstack([points, times])
        elif self.type == "NIADataset":
            nsweeps = res["lidar"]["nsweeps"]

            lidar_path = Path(info["lidar_path"])
            if 'Radar' in str(lidar_path):
                points = read_file(str(lidar_path), virtual=res["virtual"], calib=info["calib_path"], num_point_feature=5)
            else:
                points = read_file(str(lidar_path), virtual=res["virtual"], calib=info["calib_path"])
...
class LoadPointCloudAnnotations(object):
    ...
    def __call__(self, res, info):
        ...
        elif res["type"] == 'NIADataset' and "gt_boxes" in info:
            gt_boxes = info["gt_boxes"].astype(np.float32)
            gt_boxes[np.isnan(gt_boxes)] = 0
            res["lidar"]["annotations"] = {
                "boxes": gt_boxes,
