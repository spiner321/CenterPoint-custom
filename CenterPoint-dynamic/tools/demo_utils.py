...
def visual_nia(points, gt_anno, det, i, eval_range=35, conf_th=0.5, cam_path=None):
    _, axes = plt.subplots(2, 1, figsize=(9, 9), dpi=200)
    image = img.imread(cam_path)
    axes[0].imshow(image)
    ...
    axes[1].scatter(points[0, :], points[1, :], c=colors, s=0.2)
    ...
    # Show GT boxes.
    for box in boxes_gt:
        box.render(axes[1], view=np.eye(4), colors=('r', 'r', 'r'), linewidth=2)

    # Show EST boxes.
    for box in boxes_est:
        if box.score >= conf_th:
            box.render(axes[1], view=np.eye(4), colors=('b', 'b', 'b'), linewidth=1)
    ...
    plt.savefig("/path/to/CenterPoint-NIA/demo/file%02d.png" % i)
    plt.close()
...
def visual(points, gt_anno, det, i, eval_range=35, conf_th=0.5):
    ...
    plt.savefig("/path/to/CenterPoint/demo/file%02d.png" % i)
    plt.close()
