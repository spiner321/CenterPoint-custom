...
class DetectionEval:
    ...
    def __init__(self,
        ...
        self.pred_boxes, self.meta = load_prediction(self.result_path, self.cfg.max_boxes_per_sample, DetectionBox,
                                                     verbose=verbose)
        self.gt_boxes = self.load_gt_nia(DetectionBox)
    ...
    def load_gt_nia(self, DetectionBox, verbose=True):
        ...
        for idx, anno in enumerate(annos):
            ...
            for box_idx in range(len(anno['gt_boxes_token'])):
                if np.linalg.norm(anno['gt_boxes'][box_idx, 0:2]) < self.cfg.class_range[anno['gt_names'][box_idx]]:
                    sample_boxes.append(
                        DetectionBox(
                            sample_token=sample_token,
                            translation=anno['gt_boxes'][box_idx, 0:3], # xyz
                            size=anno['gt_boxes'][box_idx, 3:6], #whl
                            rotation=Quaternion(axis=[0, 1, 0], angle=anno['gt_boxes'][box_idx, 8]).elements.tolist(),
                            velocity=np.array([0, 0]),
                            detection_name=anno['gt_names'][box_idx],
...
def main():
    ...
    parser.add_argument('--dataroot', type=str, default='/path/to/CenterPoint/data/nia',
                        help='Default nuScenes data directory.')
    parser.add_argument('--version', type=str, default='v1.0-mini',
                        help='Which version of the nuScenes dataset to evaluate on, e.g. v1.0-trainval.')
    parser.add_argument('--config_path', type=str, default='/path/to/CenterPoint/configs/nia/pp/nia_centerpoint_pp_02voxel_two_pfn_1sweep.py',
